...
// Generate email content for manual sending or external services
export function generateCompanyNotificationContent(savedMessage: any): { html: string; text: string; subject: string } {
...
Newsletter: ${savedMessage.formData.newsletter ? 'Yes' : 'No'}
  ...
  const subject = `🔔 New ${methodLabel} - ${savedMessage.formData.firstName} ${savedMessage.formData.lastName}`

  return { html, text, subject }
}

// Generate user confirmation content
export function generateUserConfirmationContent(savedMessage: any): { html: string; text: string; subject: string } {
  const methodLabel = getMethodLabel(savedMessage.method)
  const { company, branding, business } = templateConfig
      ...
      <div class="content">
          ...
          <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 25px 0;">
              ...
              <ul style="margin: 0; padding-left: 20px; color: #555;">
                  ...
                  <li>💬 Reply with your reference ID: <strong style="color: #1A1A1A;">${savedMessage.referenceId}</strong></li>
              </ul>
          </div>
      </div>
...
Available ${business.workingHours} ${business.timezone}
  ...
  const subject = `✅ Message Received - We'll be in touch soon!`
...
// Log form submission (for debugging/tracking without database)
export function logFormSubmission(formData: any, referenceId: string): void {
  ...
  console.log('📧 Contact Form Submission:', JSON.stringify(logEntry, null, 2))
}